import std/[os]
import jsony # fast json library
import whisky # blocking websocket library
import malebolgia # advanced threading library
import malebolgia/lockers # for memory sharing with malebolgia

import RTR_nim_botApi2/[Defaults, Schemas, EventsQueue, Bot] # pieces of this API

export Schemas, Bot, Condition # viawable from the outside

# global variables
var
  serverUrl = getEnv("SERVER_URL", DEFAULT_SERVER_URL)
  serverSecret = getEnv("SERVER_SECRET", DEFAULT_SERVER_SECRET)
  eventsQueue = initLocker newEventsQueue()
var
  socket:WebSocket
  initialPosition = InitialPosition(x: 0, y: 0, angle: 0)

proc handleMessage(json_message: string) =
  ## **Handle the message**
  ## This method is used to handle the message received from the server.
  ## 
  ## `json_message` is the message received from the server
  let message = json2schema(json_message)

  echo "message: ", message.getType()

  case message.getType():
  of serverHandshake:
    let server_handshake = (ServerHandshake)message
    let botHandshake = BotHandshake(
        `type`: Type.botHandshake,
        sessionId: server_handshake.getSessionId(),
        name: getName(),
        version: getVersion(),
        authors: getAuthors(),
        ...
        programmingLang: getProgrammingLang(),
        initialPosition: initialPosition,
        secret: serverSecret
    )
    socket.send(botHandshake.toJson())
  of gameStartedEventForBot:
    let botReady = BotReady(`type`: Type.botReady)
    socket.send(botReady.toJson())
    lock eventsQueue as eq: eq.push(json_message)

  of roundStartedEvent:
    lock eventsQueue as eq: eq.push(json_message)

  else:
    echo "unhandled message type: ", message.getType()

proc newBot(json_file: string) =
...
proc queueCruncher(eventsQueue: Locker[EventsQueue.EventsQueue]) =
  ## **Queue Cruncher**
  ## This method is used to process the events queue
  while true:
    lock eventsQueue as evntq:
      if not evntq.isEmpty():
        let json_message = evntq.pop()
        # handleMessage(json_message)
        echo "message: ", json_message

proc start*(json_file: string) =
  ## **Start the bot**
  ## This method is used to start the bot instance.
  ## 
  ## `json_file` is the filename of the json file that contains the bot metadata
  
  newBot(json_file) # create the bot from the json file

  # start all the threads we need
  var m = createMaster()
  m.awaitAll:
    m.spawn queueCruncher(eventsQueue)

    try:
      socket = newWebSocket(serverUrl)
    except OSError:
      if getCurrentExceptionMsg() == "Connection refused":
        echo "Connection refused. Is the server running? url: ", serverUrl
      quit(0)
      ...
      try:
        ...
        if packet.isSome():
          ...
          of TextMessage:
            ...
            echo "unhandled message kind: ", pkt.kind
      except CatchableError:
        if getCurrentExceptionMsg() == "Error receiving WebSocket frame":
